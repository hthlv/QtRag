...
void DocumentRepository::update_status_and_chunk_count(const std::string &id, const std::string &status,
                                                       int chunk_count,
...
void DocumentRepository::remove_by_id(const std::string &id) {
    const char *sql = R"(
    ...
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error("prepare remove document failed");
    }
    sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_TRANSIENT);
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        throw std::runtime_error("prepare remove document failed");
